...
This plugin version branch contains the flat JSON enhancements for iOS and macOS which is to be merged into the `cordova-plugin-sqlite-evplus-ext-common-free` plugin version.
...
- This plugin version uses a flat JSON interface between Javascript and native parts on Android, iOS, and macOS to improve performance and resolve memory issues with large SQL batches
