...
(function() {
  ...
  SQLitePlugin.prototype.sqlBatch = function(sqlStatements, success, error) {
    ...
    myfn = function(tx) {
      var l, len1, results, st;
      results = [];
      for (l = 0, len1 = myStatements.length; l < len1; l++) {
        st = myStatements[l];
        if (st.constructor === Array) {
          if (st.length === 0) {
            throw newSQLError('sqlBatch array element of zero (0) length');
          }
          results.push(tx.addStatement(st[0], (st.length === 0 ? [] : st[1]), null, null));
        } else {
          results.push(tx.addStatement(st, [], null, null));
        }
      }
      return results;
    };
  ...
  SQLitePluginTransaction.prototype.run_batch_old = function(batchExecutes, handlerFor) {
